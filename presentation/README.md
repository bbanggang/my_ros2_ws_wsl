...
/scan (LaserScan)
    ...
    scanCb()
        │
        ├──► visualizeScan(scan, count) ──► 화면 출력 (cv::imshow)
        │           │
        │           │  cv::Mat image (500×500)
        │           ▼
        └──────► saveVideo(image) ──► scan_video.mp4
```
...
**입력**: `LaserScan` 데이터 | **출력**: `cv::Mat image`

**① 500×500 이미지 생성 및 로봇 위치 표시**
```cpp
...
**입력**: `cv::Mat image` (visualizeScan()의 반환값)
...
| 방향 | 미탐지 기본값 | 효과 |
|------|--------------|------|
| 좌측 | `(0, 0)` | 화면 좌측 끝으로 설정 → 통로 중심이 자동으로 좌측을 향함 |
| 우측 | `(500, 0)` | 화면 우측 끝으로 설정 → 통로 중심이 자동으로 우측을 향함 |
...
| `\|error\| > 60` | 오차가 임계값 초과 → 직진 유지 |
| `0 < \|error\| ≤ 60` | 통로 방향으로 비례 조향 (게인 `k = 0.45`) |
